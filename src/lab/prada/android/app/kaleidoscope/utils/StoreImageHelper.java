...
public class StoreImageHelper {
    ...
    private static Uri store(final ContentResolver cr, File file, String name,
            String description) {
    ...
    public static Bitmap view2Bitmap(final ContentResolver cr, final ViewGroup root) {
        Bitmap bm = Bitmap.createBitmap(root.getWidth(), root.getHeight(), Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(bm);
        canvas.drawColor(BACKGROUND_COLOR);
        for (int i = 0; i < root.getChildCount(); i++) {
            View v = root.getChildAt(i);
            canvas.drawBitmap(loadBitmapFromView(v), v.getX(), v.getY(), null);
        }
        return bm;
    }

    public static void save(final ContentResolver cr, final ViewGroup allViews,
        ...
        AsyncTask<Void, Void, Void> saveImagesTask = new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... params) {
                ...
                Bitmap mediumBitmap = Bitmap
                        .createScaledBitmap(
                                largeBitmap,
                                (int) (largeBitmap.getWidth() * Constants.MEDIUM_SCALE_FACTOR),
                                (int) (largeBitmap.getHeight() * Constants.MEDIUM_SCALE_FACTOR),
                                false);
                Bitmap smallBitmap = Bitmap
                        .createScaledBitmap(
                                largeBitmap,
                                (int) (largeBitmap.getWidth() * Constants.SMALL_SCALE_FACTOR),
                                (int) (largeBitmap.getHeight() * Constants.SMALL_SCALE_FACTOR),
                                false);
                String fileNamePrefix = UUID.randomUUID().toString();
                store(cr,
                        saveFile(largeBitmap, fileNamePrefix + "_large"
                                + IMAGE_FORMAT_TYPE), fileNamePrefix + "_large"
                                + IMAGE_FORMAT_TYPE, "");
                largeBitmap.recycle();
                store(cr,
                        saveFile(mediumBitmap, fileNamePrefix + "_medium"
                                + IMAGE_FORMAT_TYPE), fileNamePrefix + "_large"
                                + IMAGE_FORMAT_TYPE, "");
                mediumBitmap.recycle();
                store(cr,
                        saveFile(smallBitmap, fileNamePrefix + "_small"
                                + IMAGE_FORMAT_TYPE), fileNamePrefix + "_large"
                                + IMAGE_FORMAT_TYPE, "");
                smallBitmap.recycle();
                return null;
